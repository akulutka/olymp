...
struct vec //geometrical vector {dX, dY}
{
	ld dX;
	ld dY;
	vec()
...
line pToLine(point p1, point p2) //make line from 2 points
{
	ld A = p2.Y - p1.Y;
	ld B = p1.X - p2.X;
...
bool isInLine(point p, line l) //point is on line?
{
	return l.A * p.X + l.B * p.Y + l.C == 0;
}

bool isInner(point toCheck, point p1, point p2) // point is inside rectangle with diag. p1 -> p2?
{
...
ld d(point p1, point p2) //distance
{
...
bool isInside(point toCheck, vector<point> pol) //point is inside of polygon?
{
	ld res = 0;
	for (int i = 0; i < pol.size() - 1; i++)
	{
...
bool isConvex(vector<point> vP) //polygon is convex?
{
	vec v1, v2, v3;
